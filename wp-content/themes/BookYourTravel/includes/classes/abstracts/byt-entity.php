 ...
 */
 abstract class byt_entity {
	 ...
	 * @return mixed
	 */
	public function get_custom_field( $key, $use_prefix = true ) {

		// Get values or default if not set
		if ( in_array( $key, array( 'images' ) ) ) {		
			$value = ( $value = get_post_meta( $this->id, $this->get_prefix($use_prefix) . $key, true ) ) ? $value : array();			
		} elseif ( in_array( $key, array( 'is_price_per_person', 'is_self_catered', 'count_children_stay_free', 'is_price_per_group', 'price_per_day' ) )   ) {		
			$value = ( $value = get_post_meta( $this->id, $this->get_prefix($use_prefix) . $key, true ) ) ? $value : 0;
		} else {	
			$value = get_post_meta( $this->id, $this->get_prefix($use_prefix) . $key, true );			
		}
	...
	public function get_description() {
		...
		return apply_filters( 'bookyourtravel_entity_description', $content, $this );
	}

...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'tax_select':
			...
			echo '</select> &nbsp;<span class="description"><a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		break;
		// tax_checkboxes
		case 'tax_checkboxes':
			...
			echo '<span class="description">' . (isset($field['desc']) ? $field['desc'] : '') . ' <a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>';
		break;
		// date
		case 'date':
